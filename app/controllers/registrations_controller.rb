class RegistrationsController < ApplicationController
  ...
  def login
    user = Registration.find_by(Username: params[:username])
    if user && user.authenticate(params[:password])
      render "registrations/existing"
    else
      render "registrations/existing"
    end
  end
  ...
  def registration_params
    params.require(:registration).permit(:username, :phone_number, :address, :password)
  end
end
